...
export function StatCard({ title, value, percentage, information }: StatCardProps) {
  // 2. Logic to determine if the stat is positive or negative
  const isPositive = percentage >= 0;

  return (
    <div className="flex flex-col justify-between rounded-xl border-neutral-5 lg:w-[185px] lg:h-[90px] border 2xl:w-[239px] 2xl:h-[105px] shadow-md  gap-1 p-[10px]" style={{ background: 'linear-gradient(to bottom, #F6FAFE 0%, #FDFDFD 100%)' }}>
      ...
      <div className="flex w-full    h-[24px] 2xl:h-[30px]   ">
        ...
        <div className="flex  place-items-center w-full justify-between gap-[7px] ">
          ...
          <div title={information} className="cursor-help text-neutral-3 flex place-items-center hover:text-neutral-2 transition-colors">
      ...
      <div className="">
        <div className=" xl:text-[24px] 2xl:text-[32px] font-semibold text-neutral-2 tracking-tight">
          {value}
        </div>
      </div>
    </div>
